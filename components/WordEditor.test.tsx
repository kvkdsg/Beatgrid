...
describe('WordEditor Component', () => {
  ...
  it('should render 4 input fields with correctly passed words', () => {
    render(<WordEditor words={defaultWords} setWords={mockSetWords} onGenerate={mockOnGenerate} />);
  ...
  it('should call setWords when an input changes', () => {
    ...
    expect(mockSetWords).toHaveBeenLastCalledWith(['NewWord', 'Two', 'Three', 'Four']);
  });

  it('should disable generate button if any word is empty', async () => {
    const user = userEvent.setup();
    const invalidWords = ['One', '', 'Three', 'Four'];
    render(<WordEditor words={invalidWords} setWords={mockSetWords} onGenerate={mockOnGenerate} />);
