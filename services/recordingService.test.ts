...
describe('Recording Service', () => {
  ...
  it('should successfully initialize and start recording', async () => {
    ...
    const recorderInstance = (service as unknown as Record<string, unknown>).mediaRecorder as Record<string, unknown>;
    expect(recorderInstance).toBeDefined();
    
    recorderInstance.state = 'recording';
    if (recorderInstance.onstart) (recorderInstance as { onstart?: (e: Event) => void }).onstart?.(new Event('start'));
    else {
  ...
  it('should throw an error if stream cannot be captured', async () => {
    (canvas as unknown as { captureStream: undefined }).captureStream = undefined;
    (canvas as unknown as { mozCaptureStream?: unknown }).mozCaptureStream = undefined;

    const config: RecordingConfig = { canvas };
    await expect(service.startRecording(config)).rejects.toThrow(/Error capturando canvas/);
  });
