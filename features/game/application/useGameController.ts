...
import { BEATS_PER_ROUND, DEFAULT_BPM } from "../model/game.constants";
...
export function useGameController({
	...
	const clearGenErrorTimers = useCallback(() => {
		if (genErrorAutoCloseTimerRef.current)
			clearTimeout(genErrorAutoCloseTimerRef.current);
		if (genErrorFadeTimerRef.current)
			clearTimeout(genErrorFadeTimerRef.current);
	}, []);
	...
	const openGenError = useCallback(() => {
		...
		genErrorAutoCloseTimerRef.current = window.setTimeout(
			() => closeGenError(),
			5000,
		);
		...
		async (currentWords: string[]) => {
			...
			setUiState({
				uiRoundInfo: {
					round: 1,
					beat: 0,
					totalBeats: timelineRef.current.totalBeats,
				},
			...
			if (enableRecording && canvasRef.current) {
				try {
					const { CanvasRecordingService } = await import(
					...
					if (CanvasRecordingService.isSupported()) {
						...
						pendingRecordingArgsRef.current = {
							canvas: compositeCanvasRef.current || canvasRef.current,
							audioTrack,
					...
					setRecStatus({
						isRecording: false,
						error: `Error grabación: ${error instanceof Error ? error.message : String(error)}`,
					});
				}
			} else {
				isRecordingActiveRef.current = false;
			...
			if (audioRef.current) {
				...
				a.play()
					.then(() => void tryStartRecordingWithTrim())
					.catch(() => {
	...
	const handleRestartCurrentSession = useCallback(async () => {
		cancelRafIfAny();
		await ensureBackgroundCache();
		if (!spritesheetRef.current || !canvasRef.current) {
			handleReset();
			return;
		}
		resetRecordingState();
		...
		const seed =
			sessionSeedRef.current ?? Math.random().toString(36).substr(2, 9);
		sessionSeedRef.current = seed;
		timelineRef.current = createTimeline({
			words: words,
			bpm: DEFAULT_BPM,
	...
	const handleGenerate = useCallback(async () => {
		...
		setRecStatus({ isRecording: false });
		if (audioRef.current) setupAudioGraph();

		try {
			let url: string;
			if (preset) url = preset.spritesheetUrl;
			else {
				const { generateGameSpritesheet } = await import(
					"../spritesheet/geminiService"
				);
			...
			if (canvasRef.current) {
				if (!rendererRef.current) {
					...
					const RendererClass =
						CanvasRendererClassRef.current ||
						(await import("../rendering/renderer")).CanvasRenderer;
