...
export function useShareVideo(recordedBlob?: Blob, mimeType?: string) {
  ...
  const canShareNative = useMemo(() => {
    if (!recordedBlob || !mimeType || typeof navigator === 'undefined' || !navigator.canShare) return false;
    
    try {
      const cleanMime = mimeType.toLowerCase().includes('mp4') ? 'video/mp4' : 'video/webm';
      const file = new File([recordedBlob], `BeatGrid_Gameplay.${cleanMime.split('/')[1]}`, { 
        type: cleanMime 
      });
