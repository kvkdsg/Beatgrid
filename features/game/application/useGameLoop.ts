...
import {
  DEFAULT_BPM, SONG_OFFSET_SEC, CELL_LABEL_DURATION_MS,
  ...
  RECORDING_VIDEO_LAG_MS, RECORDING_TRIM_END_MS
} from '../model/game.constants';
import { UiState } from './useGameController';
...
interface GameLoopDeps {
  ...
  syncRef: React.MutableRefObject<{ startTime: number; visualOffset: number; isLocked: boolean; driftSamples: number[]; lastAudioTime: number; lastFrameTime: number; }>;
  startTimeRef: React.MutableRefObject<number | null>;
  applyCornerDance: (timeMs: number) => void;
  resetCornerTransform: () => void;
  renderCompositeFrame: (timeMs: number, opts?: { forceIntro?: boolean; introTextOverride?: string; gameStateOverride?: Partial<GameState> | null }) => void;
  getReadyText: string;
  cancelRafIfAny: () => void;
...
export function useGameLoop({
  appState, isMobileVertical, setUiState, 
  sessionRefs: { timelineRef, engineStateRef, spritesheetRef, rendererRef, CanvasRendererClassRef },
  recFlow: { isRecordingActiveRef, recordingStartedRef, recordingStoppedRef, recordingServiceRef, setStatus: setRecStatus },
  canvasRef, viewCanvasRef, canvasContainerRef,
  audioRef, requestRef, layoutAnimRef, lastLayoutProgressRef, syncRef, startTimeRef,
  applyCornerDance, resetCornerTransform, renderCompositeFrame, getReadyText,
  cancelRafIfAny, performAudioLock
}: GameLoopDeps) {
  ...
  const animate = useCallback(function loop() {
    ...
    const targetLayout = isMobileVertical ? 1 : 0; const diff = targetLayout - layoutAnimRef.current;
    if (Math.abs(diff) > 0.001) layoutAnimRef.current += diff * 0.1; else layoutAnimRef.current = targetLayout;
    ...
    if (visualTimeMs < offsetMs) {
      ...
      rendererRef.current.render(canvasRef.current.width, canvasRef.current.height, timelineRef.current.rounds[0].pattern, timelineRef.current.rounds[0].pattern, 0, 0, -1);
      projectToViewCanvas(layoutAnimRef.current); requestRef.current = requestAnimationFrame(loop); return;
    }
    ...
    if (isRecordingActiveRef.current && recordingStartedRef.current && !recordingStoppedRef.current && timelineRef.current) {
      if (gameTimeMs >= Math.max(0, timelineRef.current.totalBeats * timelineRef.current.msPerBeat - RECORDING_TRIM_END_MS)) {
        ...
        const activeRecorder = recordingServiceRef.current; recordingServiceRef.current = null; isRecordingActiveRef.current = false;
    ...
    if (state.isFinished) {
      ...
      if (isRecordingActiveRef.current && !recordingStoppedRef.current) {
          ...
          const activeRecorder = recordingServiceRef.current; recordingServiceRef.current = null; isRecordingActiveRef.current = false;
          if (activeRecorder) activeRecorder.stopRecording().then((r) => setRecStatus({ isRecording: false, recordedUrl: r.url, recordedBlob: r.blob, mimeType: r.mimeType })).catch(() => setRecStatus({ isRecording: false, error: 'Error guardando video.' }));
      } else if (!recordingStoppedRef.current) setRecStatus({ isRecording: false });
    ...
    projectToViewCanvas(layoutAnimRef.current); requestRef.current = requestAnimationFrame(loop);
  },[appState, isMobileVertical, cancelRafIfAny, applyCornerDance, resetCornerTransform, renderCompositeFrame, getReadyText, performAudioLock, projectToViewCanvas, audioRef, isRecordingActiveRef, recordingStartedRef, recordingStoppedRef, timelineRef, spritesheetRef, engineStateRef, rendererRef, CanvasRendererClassRef, layoutAnimRef, syncRef, startTimeRef, canvasRef, setUiState, requestRef, setRecStatus, recordingServiceRef]);
