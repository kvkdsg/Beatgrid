...
interface CompositeFrameRendererDeps {
	...
	computeCornerDance: (timeMs: number) => {
		scale: number;
		y: number;
		rot: number;
	};
...
export function useCompositeFrameRenderer({
	...
	const renderCompositeFrame = useCallback(
		(
			...
			if (cachedBgCanvasRef.current)
				ctx.drawImage(cachedBgCanvasRef.current, 0, 0);
			else {
			...
			if (
				currentState?.showCellLabels &&
				currentState.cellLabelsAlpha &&
				currentState.cellLabelsAlpha > 0
			) {
				...
				if (Array.isArray(pattern) && pattern.length > 0) {
					...
					pattern.forEach((q, i) => {
						...
						const y = row * cellH + (cellH - cellH * 0.12);
						...
						const bgW = textWidth + 32;
						const bgH = cellH * 0.14 + 16;
						...
						else ctx.rect(x - bgW / 2, y - bgH / 2, bgW, bgH);
						ctx.fill();
						ctx.stroke();
						ctx.fillStyle = "black";
			...
			const fontPx = Math.round(H * 0.12);
			...
			const drawEmoji = (
				...
				ctx.save();
				ctx.translate(x, yBase + y);
				ctx.rotate(((baseRot + rot * mirror) * Math.PI) / 180);
			...
			drawEmoji("🎉", margin + 50, margin + 50, -15, 1);
			drawEmoji("🎁", W - margin - 50, margin + 50, 15, -1);
			drawEmoji("😎", margin + 50, H - margin - 50, -10, 1);
			drawEmoji("🐸", W - margin - 50, H - margin - 50, 10, -1);

			ctx.save();
			const markFontSize = Math.round(H * 0.035);
			ctx.font = `900 ${markFontSize}px 'Montserrat', sans-serif`;
			ctx.textAlign = "center";
			...
			ctx.fillText("BEAT.BOTON.ONE", W / 2, H - Math.round(H * 0.04));
			ctx.restore();

			const activeIntro = opts?.forceIntro ?? isInIntro;
			const activeState =
				opts?.gameStateOverride ||
				(appState === AppState.PLAYING ? engineStateRef.current : null);

			if (!activeIntro && activeState && activeState.roundNumber) {
				ctx.save();
				const hudScale = H / 1080;
				const safeRound = Math.min(
					5,
					Math.max(1, Math.floor(activeState.roundNumber)),
				);
				const labelText = canvasLabelsRef.current.roundLabel;
				const valText = safeRound.toString();
				const totalText = "/5";
				...
				ctx.font = valFont;
				const valM = ctx.measureText(valText);
				ctx.font = totalFont;
				const totalM = ctx.measureText(totalText);
				const gap1 = 12 * hudScale;
				const gap2 = 4 * hudScale;
				const padX = 24 * hudScale;
				const padY = 12 * hudScale;
				const badgeW =
					padX + labelM.width + gap1 + valM.width + gap2 + totalM.width + padX;
				const badgeH =
					Math.max(
						labelM.actualBoundingBoxAscent + labelM.actualBoundingBoxDescent,
						valM.actualBoundingBoxAscent + valM.actualBoundingBoxDescent,
					) +
					padY * 2;
				const badgeX = (W - badgeW) / 2;
				const badgeY = Math.max(20, H * 0.05);
				ctx.fillStyle = "rgba(0,0,0,1)";
				ctx.beginPath();
				if (ctx.roundRect)
					ctx.roundRect(badgeX + 4, badgeY + 4, badgeW, badgeH, 16 * hudScale);
				else ctx.rect(badgeX + 4, badgeY + 4, badgeW, badgeH);
				ctx.fill();
				...
				ctx.beginPath();
				if (ctx.roundRect)
					ctx.roundRect(badgeX, badgeY, badgeW, badgeH, 16 * hudScale);
