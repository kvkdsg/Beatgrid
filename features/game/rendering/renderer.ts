import { QUADRANT_SIZE, SPRITE_RESOLUTION } from "../model/game.constants";
import type { BoardState } from "../model/game.types";
...
export class CanvasRenderer {
	...
	constructor(canvas: HTMLCanvasElement, spritesheet: SpriteSource) {
		...
		const width =
			"width" in this.spritesheet
				? this.spritesheet.width
				: (this.spritesheet as HTMLImageElement).naturalWidth;
		const height =
			"height" in this.spritesheet
				? this.spritesheet.height
				: (this.spritesheet as HTMLImageElement).naturalHeight;

		if (
			width &&
			height &&
			(width !== SPRITE_RESOLUTION || height !== SPRITE_RESOLUTION)
		) {
			console.warn(
				`[CanvasRenderer] Spritesheet dimensions mismatch. Got ${width}x${height}, expected ${SPRITE_RESOLUTION}.`,
			);
		}
	...
	public render(
		...
		for (let i = 0; i < len; i++) {
			...
			if (isActive) {
				...
				if (interp > 0)
					this.drawCell(prevPattern[i], drawX, drawY, drawW, drawH, interp);
				this.drawCell(
					currentPattern[i],
					drawX,
					drawY,
	...
	private clampQuadrantIndex(q: number) {
