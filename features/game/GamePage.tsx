...
import { RECORDING_VIDEO_LAG_MS, RECORDING_TRIM_START_MS } from './model/game.constants';
import { pickRandomPreset, PRESETS, wordsKey } from './model/presets';
...
import { useGameController, UiState, initialUiState } from './application/useGameController';
...
const GamePage: React.FC = () => {
  ...
  const { enableRecording, setEnableRecording, status: recStatus, setStatus: setRecStatus } = recFlow;
  const { recordingStartedRef, recordingServiceRef, pendingRecordingArgsRef, isRecordingActiveRef } = recFlow;

  const { canShareNative, handleShareVideo } = useShareVideo(recStatus.recordedBlob, recStatus.mimeType);
  ...
  const { cornerTLRef, cornerTRRef, cornerBLRef, cornerBRRef, computeCornerDance, resetCornerTransform, applyCornerDance } = useCornerAnimation();
  ...
  const { renderCompositeFrame } = useCompositeFrameRenderer({
    ...
    normalizedWords, canvasLabelsRef, appState: uiState.appState, isInIntro: uiState.isInIntro, introText: uiState.introText,
    engineStateRef, computeCornerDance
  });

  const tryStartRecordingWithTrim = useCallback(async function loop() {
    if (recordingStartedRef.current) return;
    const a = audioRef.current; const recorder = recordingServiceRef.current; const args = pendingRecordingArgsRef.current;
    ...
    recordingStartedRef.current = true; isRecordingActiveRef.current = true;
  ...
  },[getReadyText, renderCompositeFrame, waitNextAnimationFrame, audioRef, recordingStartedRef, recordingServiceRef, pendingRecordingArgsRef, isRecordingActiveRef, syncRef, setRecStatus]);
  ...
  const {
    closeGenError, handleReset,
    handleRestartCurrentSession, handleGenerate
  } = useGameController({
    words, uiState, setUiState, audioRef, audioContextRef, setupAudioGraph, recFlow, sessionRefs,
    ensureBackgroundCache, loadImage, prepareSpriteSource, 
    tryStartRecordingWithTrim,
    performAudioLock, cancelRafIfAny, resetCornerTransform, canvasRef, compositeCanvasRef, startTimeRef, syncRef, layoutAnimRef
  });

  useGameLoop({
    appState: uiState.appState, isMobileVertical: uiState.isMobileVertical, setUiState, sessionRefs, recFlow,
    canvasRef, viewCanvasRef, canvasContainerRef, audioRef, requestRef, layoutAnimRef, lastLayoutProgressRef,
    syncRef, startTimeRef, applyCornerDance, resetCornerTransform, renderCompositeFrame, getReadyText,
