        ...
        <div className="relative bg-white p-5 md:p-8 border-[6px] border-black hard-shadow flex flex-col items-center gap-6 max-w-4xl w-full box-border animate-slam origin-center">
          ...
          {showVideoSection && (
            <div className="w-full flex flex-col items-center animate-delay-2">
              <div className="w-full md:w-80 mx-auto border-4 border-black bg-zinc-950 relative aspect-video shadow-inner">
                <video src={recordedUrl} controls autoPlay loop className="w-full h-full object-contain" />
              </div>
            </div>
          )}
          ...
          <div className="flex flex-col w-full gap-4 mt-2 animate-delay-3">
            <div className="flex flex-col sm:flex-row gap-3 w-full justify-center">
              {canShareNative && (
                <button type="button" onClick={onShareVideo} className="w-full sm:w-auto justify-center bg-[#00ff99] text-black border-4 border-black px-6 py-3 text-sm md:text-lg font-black uppercase hard-shadow hard-shadow-hover transition-all flex items-center gap-2 hover:bg-[#00cc7a]">
                  <span>🚀</span><span>{t('actions.share')}</span>
                </button>
              )}
              {recordedUrl && (
                <a href={recordedUrl} download={`BeatGrid_Session_${new Date().getTime()}${mimeType?.includes('mp4') ? '.mp4' : '.webm'}`} className="w-full sm:w-auto justify-center bg-white text-black border-4 border-black px-6 py-3 text-sm md:text-lg font-black uppercase hard-shadow hard-shadow-hover transition-all flex items-center gap-2 hover:bg-gray-50">
                  <span>⬇</span><span>{t('actions.download')}</span>
                </a>
              )}
              <button type="button" onClick={onRestartSession} className="w-full sm:w-auto bg-white text-black border-4 border-black px-6 py-3 text-sm md:text-lg font-black uppercase hard-shadow hard-shadow-hover transition-all hover:bg-gray-50">
                {t('actions.restart')}
              </button>
              <button type="button" onClick={onReset} className="w-full sm:w-auto bg-[#ffe600] text-black border-4 border-black px-6 py-3 text-sm md:text-lg font-black uppercase hard-shadow hard-shadow-hover transition-all hover:bg-[#ffd500]">
                {t('actions.new')}
              </button>
            </div>
