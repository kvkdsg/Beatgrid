...
const MainMenu: React.FC<MainMenuProps> = ({
	...
	return (
		<div
			className={`absolute inset-0 z-30 w-full h-full overflow-y-scroll overflow-x-hidden no-scrollbar layer-transition ${show ? "visible-layer" : "hidden-layer"}`}
		>
			<div className="min-h-full w-full flex flex-col items-center justify-center py-8 px-4 relative">
				...
				{showGenError && (
					...
					>
						<span
							className="text-white text-xl md:text-2xl shrink-0"
							aria-hidden="true"
						>
							⚠️
						</span>
						<span
							className="text-white font-black text-xs md:text-sm uppercase tracking-widest flex-1 text-left"
				...
				<div className="relative z-30 flex flex-col items-center gap-4 md:gap-8 text-center p-5 md:p-8 bg-white rounded-3xl border-4 md:border-6 border-black hard-shadow w-full max-w-2xl mt-12 md:mt-0">
					<div className="flex items-center justify-between w-full gap-2 md:gap-4 mb-2 md:mb-0">
						<NavArrow
							...
							ariaLabel={t("nav.prevPresetAria")}
						/>
						<div className="flex flex-col gap-1 items-center transform -rotate-2 mx-auto">
							...
							<h2 className="text-xl md:text-3xl font-black text-white bg-[#ff0055] px-4 py-1 tracking-widest uppercase border-4 border-black inline-block transform rotate-1">
								{t("hero.subtitle")}
							</h2>
						</div>
						<NavArrow
							...
							disabled={isGenerating}
							ariaLabel={t("nav.nextPresetAria")}
						/>
					</div>
