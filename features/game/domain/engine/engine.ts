...
export const getGameStateAtTime = (
	...
	const currentRoundIdx =
		beatsPerRound > 0 ? (currentBeat / beatsPerRound) | 0 : 0;
	const lastRoundIdx = timeline.rounds.length - 1;
	const safeRoundIdx =
		currentRoundIdx < 0
			? 0
			: currentRoundIdx > lastRoundIdx
				? lastRoundIdx
				: currentRoundIdx;
	...
	const nextRound =
		safeRoundIdx + 1 < timeline.rounds.length
			? timeline.rounds[safeRoundIdx + 1]
			: currentRound;
	...
	const transitionStart = Math.max(
		0,
		PATTERN_SWITCH_BEAT - safeTransitionBeats,
	);
	...
	if (
		safeTransitionBeats > 0 &&
		roundBeatOffset >= transitionStart &&
		roundBeatOffset < PATTERN_SWITCH_BEAT
	) {
	...
	if (CELL_LABEL_DURATION_MS > 0 && safeRoundIdx < lastRoundIdx) {
		...
		const labelStartMs =
			(currentRoundStartBeat + PATTERN_SWITCH_BEAT) * msPerBeat;
		const activeStartMs = nextRoundStartBeat * msPerBeat;
		const labelEndMs = Math.min(
			labelStartMs + CELL_LABEL_DURATION_MS,
			activeStartMs,
		);
		...
		if (timeMs >= labelStartMs && timeMs < labelEndMs) {
			...
			const alphaIn =
				CELL_LABEL_FADE_IN_MS > 0
					? clamp01(timeSinceStart / CELL_LABEL_FADE_IN_MS)
					: 1;
			const timeUntilEnd = labelEndMs - timeMs;
			const alphaOut =
				CELL_LABEL_FADE_OUT_MS > 0
					? clamp01(timeUntilEnd / CELL_LABEL_FADE_OUT_MS)
					: 1;
